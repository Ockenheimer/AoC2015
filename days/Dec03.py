...
trace = list(open("days/input03", "r").read())
traceSanta = trace[::2]
traceRobo = trace[1::2]
...
positionSanta = [posX1, posY1]
positionRobo = [posX2, posY2]
giftsReceived = [[0, 0]]

for direction in traceSanta:
    if direction == ">":
        posY1 += 1
    elif direction == "<":
        posY1 -= 1
        ...
        posX1 -= 1
    positionSanta = [posX1, posY1]
    if positionSanta not in giftsReceived:
        giftsReceived.append(positionSanta)

for direction in traceRobo:
    if direction == ">":
        posY2 += 1
    elif direction == "<":
        posY2 -= 1
        ...
        posX2 -= 1
    positionRobo = [posX2, posY2]
    if positionRobo not in giftsReceived:
        giftsReceived.append(positionRobo)



print("hier könnte ihre WErbung stehen " + str(len(giftsReceived)))
