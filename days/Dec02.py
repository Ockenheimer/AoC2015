...
fileDec02 = open("days/input02", "r")

listOfPresents = fileDec02.read().split("\n")
...
iterations = 0
for present in listOfPresents:
    measures = list(map(int, present.split("x")))
    side1 = measures[0] * measures[1]
    side2 = measures[1] * measures[2]
    side3 = measures[0] * measures[2]
    length1 = measures[0]
    length2 = measures[1]
    length3 = measures[2]
    measuresWithoutMax = measures
    measuresWithoutMax.remove(max(measures))
    totalRibbonSize += 2 * measuresWithoutMax[0] + 2 * measuresWithoutMax [1]
    totalRibbonSize += length1 * length2 * length3
    totalSquareFeet += 2 * side1 + 2 * side2 + 2 * side3
    totalSquareFeet += min(side1, side2, side3)
    iterations += 1

print(totalSquareFeet)
print(totalRibbonSize)
